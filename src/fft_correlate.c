...
void fill_pad_4d_array(
  ...
  float *arr_l, float *padded_arr_l
    ...
    for (int i = 0; i < n_grids; i++) {
        float *cur_padded_arr_l = padded_arr_l + (pad_x * pad_y * pad_z * i);
        float *cur_arr_l = arr_l + (x * y * z * i);
...
void excute_fft_correlate(
  ...
  for (int i = 0; i < n_grids; i++) {
    ...
    for (int j = 0; j < n_orientations; j++) {
      ...
      fftwf_execute_dft_c2r(plan_inv, fft_l, cur_corr);
    }
  }
}
...
void fft_correlate(
  ...
  plan_inv = fftwf_plan_dft_c2r_3d(nx, ny, nz, fft_l, corr, FFTW_ESTIMATE);
...
void fft_correlate_batch(
  ...
  // Create forward and inverse FFT plans for both arrays
  plan_fwd = fftwf_plan_dft_r2c_3d(nx, ny, nz, arr_r, fft_r, FFTW_MEASURE);
  plan_inv = fftwf_plan_dft_c2r_3d(nx, ny, nz, fft_l, corr, FFTW_MEASURE);
