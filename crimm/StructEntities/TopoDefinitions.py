...
class ResidueDefinition:
    ...
    def build_standard_coord_from_ic_table(self):
        ...
        resname = self.resname.split('+')[0] # remove possible patch from the resname
        if resname in aa_3to1:
            base_ic1 = self.ic[('-C', 'N', 'CA', 'C')]
            base_ic2 = self.ic[('N', 'CA', 'C', '+N')]
            init_atoms = ('-C', 'N', 'CA')
        elif resname in nucleic_letters_3to1_extended:
            warnings.warn(
            ...
            warnings.warn(
                f'Unknown Residue Type {resname}! '
                'Standard coord build from IC table is skipped.'
            )
            base_ic1, base_ic2 = list(self.ic.values())[:2]
            init_atoms = list(self.ic.keys())[0][:3]
            return
        ...
        self.standard_coord_dict = {
            init_atoms[0]: np.array([r_CN,0,0], dtype=float),
            init_atoms[1]: np.array([0, 0, 0], dtype=float),
            init_atoms[2]: np.array([x_N, y_N, 0])
        }
